...
(function($){
    $.extend(true,$.summernote.lang,{
        'en-US':{
            keywords:{
                tooltip:'Keywords',
                notification:'Keywords Grabbed, you should review them!!!'
            }
        }
    });
    ...
    $.extend($.summernote.plugins,{
        'keywords':function(context){
            ...
            $('.note-statusbar').css({'position':'relative','height':'14px'});
            context.memo('button.keywords',function(){
                var button=ui.button({
                    contents:options.keywords.icon,
                    tooltip:lang.keywords.tooltip,
                    click:function(){
                        var text=getKeywords($(options.keywords.element).text());
                        if(options.keywords.outType=='input'){
                            if(options.keywords.action=='replace'){
                                $(options.keywords.outElement).val(text);
                            }else{
                                var prependText=$(options.keywords.outElement).val();
                                $(options.keywords.outElement).val(prependText+','+text);
                            }
                            if(options.keywords.triggerInput==true)$(options.keywords.outElement).trigger("change");
                        }else{
                            if(options.keywords.action=='replace'){
                                $(options.keywords.outElement).html(text);
                            }else{
                                var prependText=$(options.keywords.outElement).html();
                                $(options.keywords.outElement).html(prependText+','+text);
                            }
                        }
                        $('.note-resizebar').append('<div class="keywordsAlert" style="position:absolute;background-color:#0f0;bottom:0;left:2px;font-size:10px;color:#000;">'+lang.keywords.notification+'</div>');
                        setTimeout(function(){$('.keywordsAlert').remove();},options.keywords.time);
                    }
                });
